...
final class FieldTimeApplicationService {
  ...
  Future<FieldTimeSnapshot> saveReceipt({
    ...
    final receipt = Receipt(
      ...
      tax: tax,
      receiptNumber:
          receiptNumber?.trim().isEmpty == true ? null : receiptNumber?.trim(),
    ...
    return snapshot.copyWith(
      ...
      syncQueue: [
        ...
        _queue(snapshot, 'ReimbursementRequest', reimbursement.id,
            SyncOperation.create, now),
      ],
    );
  }
  ...
  FieldTimeSnapshot addJobPhoto({
    ...
    return snapshot.copyWith(
      ...
      syncQueue: [
        ...snapshot.syncQueue,
        _queue(
            snapshot, 'Attachment', attachment.id, SyncOperation.create, now),
      ],
    );
  }
  ...
  FieldTimeSnapshot addObservation({
    ...
    return snapshot.copyWith(
      ...
      syncQueue: [
        ...snapshot.syncQueue,
        _queue(snapshot, 'WorkSegment', updatedSegment.id, SyncOperation.update,
            at),
      ],
    );
  }
  ...
  List<WorkDay> timesheet(
    ...
    final start = switch (period) {
      ...
      TimesheetPeriod.week => DateTime(now.year, now.month, now.day)
