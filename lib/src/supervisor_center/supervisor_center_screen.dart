...
enum _JobsOverviewPeriod { day, week, month, year, all }
              ...
              children: [
                ...
                SingleChildScrollView(
                  ...
                  child: SegmentedButton<_JobsOverviewPeriod>(
                    ...
                    segments: const [
                      ButtonSegment(
                        value: _JobsOverviewPeriod.day,
                        label: Text('Dia'),
                      ),
                      ButtonSegment(
                        value: _JobsOverviewPeriod.week,
                        label: Text('Semana'),
                      ),
                      ButtonSegment(
                        value: _JobsOverviewPeriod.month,
                        label: Text('Mês'),
                      ),
                      ButtonSegment(
                        value: _JobsOverviewPeriod.year,
                        label: Text('Ano'),
                      ),
                      ButtonSegment(
                        value: _JobsOverviewPeriod.all,
                ...
                if (rankedJobs.isEmpty)
                  const JkddEmptyState(
                    ...
                    message:
                        'Cadastre uma obra para começar a acompanhar as horas.',
                  )
                else
                  _JobsHoursChart(
                    jobs: rankedJobs,
...
final class _InteractiveSummaryCard extends StatelessWidget {
...
List<_JobActivityBucket> _jobActivityBuckets(
  ...
  return List.generate(12, (index) {
    ...
    final hours = entries
        .where(
          (entry) => entry.date.year == now.year && entry.date.month == month,
        )
        .fold<double>(0, (sum, entry) => sum + _entryHoursValue(entry));
...
double _entryRegularHoursValue(TimeEntry entry) =>
    (_entryHoursValue(entry) - entry.travelBonusHours).clamp(
      0.0,
      double.infinity,
    );
